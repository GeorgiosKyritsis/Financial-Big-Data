...
df_min_variance = data.frame(matrix(ncol = 4, nrow = length(alldates)))
colnames(df_min_variance) = c('pmPD', 'psPD', 'pmLW', 'psLW')
rownames(df_min_variance) = alldates

setwd("/Users/kyritsis/Desktop/Financial Big Data/final/Data/portfolio_attributes_min_variance/")

## In sample Expected returns and standard deviations

for (day in alldates) {
  fname = paste0("port_PD_", day,".rds")
  temp = readRDS(fname)
  df_min_variance[day,1] = temp$pm
  df_min_variance[day,2] = temp$ps
  print(day)
}

for (day in alldates) {
  fname = paste0("port_LW_", day,".rds")
  temp = readRDS(fname)
  df_min_variance[day,3] = temp$pm
  df_min_variance[day,4] = temp$ps
  print(day)
}

expected_return_std = as.xts(df_min_variance)
setwd("/Users/kyritsis/Desktop/Financial Big Data/final")


png("figures/fig_min_variance_ins_r.png", width = 5, height = 4, units = 'in', res = 300)
...
png("figures/fig_min_variance_ins_std.png", width = 5, height = 4, units = 'in', res = 300)
ggplot(expected_return_std, aes(index(expected_return_std))) + 
...
df_out_of_sample_min_variance = data.frame(matrix(ncol = 4, nrow = length(alldates[1:2049])))
colnames(df_out_of_sample_min_variance) = c('pmPD', 'psPD', 'pmLW', 'psLW')
rownames(df_out_of_sample_min_variance) = alldates[1:2049]
...
for (day in alldates[1:2049]) {
  ...
  pm = mean(yy)
  ps = sd(yy)
  df_out_of_sample_min_variance[day,1] = pm
  df_out_of_sample_min_variance[day,2] = ps
  index = index + 1
  print(day)
}
...
for (day in alldates[1:2049]) {
  ...
  pm = mean(yy)
  ps = sd(yy)
  df_out_of_sample_min_variance[day,3] = pm
  df_out_of_sample_min_variance[day,4] = ps
  index = index + 1
  print(day)
}

out_of_sample_expected_return_std = as.xts(df_out_of_sample_min_variance)
setwd("/Users/kyritsis/Desktop/Financial Big Data/final")

png("figures/fig_min_variance_ous_r.png", width = 5, height = 4, units = 'in', res = 300)
...
png("figures/fig_min_variance_ous_std.png", width = 5, height = 4, units = 'in', res = 300)
ggplot(out_of_sample_expected_return_std, aes(index(out_of_sample_expected_return_std))) + 
...
png("figures/fig_min_variance_cum.png", width = 5, height = 4, units = 'in', res = 300)
